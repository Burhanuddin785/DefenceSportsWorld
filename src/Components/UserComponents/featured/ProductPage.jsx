...
useEffect(()=>{
    axios.get(`https://api.defencesportsworld.com/api/products/${productID}`).then((res)=> {setProduct(res.data[0]);  console.log(res.data[0])}).catch(err=> console.log(err))
},[productID])
    ...
    <div className="topSec">
        <div className="imageList">
            {product?.images?.map((limage, index)=>(
                <div key={index} className={`image ${index === clicked  ? "clicked" : ""}`} ><img src={`https://api.defencesportsworld.com/adminUploads/products/${product.subCategory._id}/${limage.filename}`} onClick={()=>{setClicked(index)}} alt="image" /></div>
            ))}
        </div>
        <div className="productImage"> <img src={`https://api.defencesportsworld.com/adminUploads/products/${product?.subCategory?._id}/${product?.images?.[clicked]?.filename}`} alt="image" /> </div>
